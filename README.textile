...
In it's simplest form, we can do the following:
...
@book_conductor.authorships = {
  0 => { :role => "" },                         # Record doesn't exist (no id field) and won't be created (no author_id field)
  1 => { :id => 5, :role => "Lead Writer" },    # Record is currently associated, but will be removed (no author_id field)
  2 => { :author_id => 34, :role => "Editor" }  # Record doesn't exist (no id field), but will be created and associated with the book
}</code></pre>
