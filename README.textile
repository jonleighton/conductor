...
Conductor is a "Rails":http://rubyonrails.org/ plugin which *helps you manage creating and updating a record and its associations via a single form submission*.

It does this by:

* Allowing complex nested parameters to represent associations
* Creating, finding or deleting the associated objects as necessary
* Only performing database changes when told to and within a transaction
* Aggregating error messages occurring on associated objects
...
* Clear, concise documentation is lacking
* It only works with PostgreSQL

However:

* It has almost 100% RSpec coverage
* It has been used and developed on a large, real-world project which has received extensive testing
* There is an "example app":https://github.com/jonleighton/conductor_example/tree available for inspection

The current focus is to improve the documentation and allow this to drive API changes so the plugin can be more easily understood.
...
A conductor initially seems to be similar to a "presenter":http://www.jayfields.com/rails_presenter.htm, particular as the presenter example Jay Fields gives does manage multiple objects in order to keep the controller concise. However, the distinction in my mind is this:
...
*Please note*: This documentation was written a while ago as internal documentation for the project for which Conductor was developed. It is written more like and tutorial and so will make most sense if read from start to finish. I am aware it ought to be substantially easier to grasp the core concepts and this is a general failing of the plugin at the moment - something I would like to fix.
...
h3. Conducting one-to-many associations
...
There should probably be, and perhaps is, a way to use has_and_belongs_to_many associations with a Conductor, but because we are using @:through@ associations exclusively, I will focus on how to use the Conductor plugin with those.
